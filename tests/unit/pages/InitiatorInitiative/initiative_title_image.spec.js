...
beforeEach(() => {
  ...
  wrapper = shallowMount(InitiativeTable, {
    ...
    localVue,
    router,
    stubs: ['base-input', 'base-button', 'router-link', 'initiative-title-image', 'base-spinner'],
  })
...
describe('Initiator Initiative', () => {
  ...
  it('has initiative-item divs that are not empty for valid initiatives data', () => {
    const initiative_item = wrapper.findAll('.initiatives > div > .initiative-item')
  ...
  it('has initiative info with title and id', () => {
    ...
    expect(initiative_title.text()).toContain(wrapper.vm.$data.initiatives[0].initiative_title)
    expect(initiative_title.text()).toContain(wrapper.vm.$data.initiatives[0].initiative_id)
  })
  ...
  // it('has a edit button with router link to /initiatorinitiativeform', () => {
  //   const edit_button = wrapper.findAll('.initiative-link').at(3)
  //   expect(edit_button.exists()).toBe(true)
  //   const router_link = edit_button.find('router-link-stub')
  //   expect(router_link.exists()).toBe(true)
  //   expect(router_link.attributes().to).toBe('/initiatorinitiativeform')
  // })

  // it('has a messages button with router link to /initiatormessages', () => {
  //   const messages_button = wrapper.findAll('.initiative-link').at(4)
  //   expect(messages_button.exists()).toBe(true)
  //   const router_link = messages_button.find('router-link-stub')
  //   expect(router_link.exists()).toBe(true)
  //   expect(router_link.attributes().to).toBe('/initiatormessages')
  // })

  // it('has access key button which calls openModal when clicked on', async () => {
  //   let acceskey_div = wrapper.findAll('.initiative-link').at(5)
  ...
  //   await acceskey_button.vm.$emit('click')
...
describe('Initiator Initiative Modal', () => {
  ...
  it('calls sendInstallerKey method when send access keys button clicked', async () => {
    ...
    await wrapper.vm.$nextTick()
    const sendInstallerKeyBtn = wrapper.find('form:first-of-type > base-button-stub')
    ...
    wrapper.setData({
      showModal: true,
      installers: [{ installer_name: 'new installer', initiative_installer_id: '8', status: 'confirmation awaited' }],
    })
    await wrapper.vm.$nextTick()
  ...
  it('displays resend key forms when installer and advisor status are "confirmation awaited"', async () => {
    ...
    expect(wrapper.find('.modal-form:first-of-type > div').exists()).toBe(false)
  ...
  it('displays divs with infos when installer and advisor status are "verified"', async () => {
    ...
    wrapper.setData({
      installers: [{ installer_name: 'new installer', initiative_installer_id: '8', status: 'verified' }],
    })
    await wrapper.vm.$nextTick()
    expect(wrapper.findAll('form').length).toBe(3)
    ...
    expect(wrapper.findAll('.modal-body > div').at(0).text())
